...
const MOCK_BALANCE = "2.23";
const MOCK_ETH_PRICE_USD = 702.63; // per ETH

export const SendEnterAmount = () => {
  const navigation = useNavigation<NativeStackNavigationProp<WalletStackParamList>>();
  const [amount, setAmount] = useState("");
  const inputRef = useRef<TextInput>(null);

  const numericAmount = parseFloat(amount) || 0;
  const availableBalance = parseFloat(MOCK_BALANCE);
  const canSend = numericAmount > 0 && numericAmount <= availableBalance;

  const displayAmount = amount || "0";

  const handleMax = () => {
    setAmount(MOCK_BALANCE);
  };

  const handleClearAddress = () => {
    // Clear address / go back to address entry
    navigation.goBack();
  };
        ...
        >
          ...
          <View className="flex-1 items-center justify-center px-4">
            ...
            >
              ...
              {/* USD value */}
              <Text className="mt-2 text-white/50" style={styles.usdText}>
                US${(numericAmount * MOCK_ETH_PRICE_USD).toFixed(2)}
              </Text>
            </Pressable>
          </View>
